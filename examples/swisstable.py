from juniper import SwissTable, RHMap
import random
import time
...
print("\n=== Создание из dict ===")
m2 = SwissTable({"a": 1, "b": 2, "c": 3})
print(f"SwissTable(dict) = {m2}")

print("\n=== Создание из пар ===")
m3 = SwissTable([("x", 10), ("y", 20)])
print(f"SwissTable(pairs) = {m3}")

print("\n=== Сравнение == / != ===")
a = SwissTable({"k": 1, "v": 2})
b = SwissTable({"v": 2, "k": 1})
c = SwissTable({"k": 1})
print(f"a == b -> {a == b}")
print(f"a != c -> {a != c}")
...
for i in range(99_900, 100_000):
    assert i in big
    assert big[i] == i * 2
print("Shrink корректен")

print("\n=== clear ===")
big.clear()
print(f"После clear: len = {len(big)}")

print("\n=== Бенчмарк: SwissTable vs RHMap ===")
N = 100_000
data = list(range(N))
random.shuffle(data)
lookup = [random.randint(0, N - 1) for _ in range(N)]

for name, cls in [("SwissTable", SwissTable), ("RHMap", RHMap)]:
    ...
    print(f"  {name:12s}: insert {insert_ms:.1f} ms, lookup {lookup_ms:.1f} ms")

print("\n=== del ===")
tmp = SwissTable({"temp": 1})
del tmp
print("tmp удалён")
