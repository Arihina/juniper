...
typedef enum { SWI_KEYS, SWI_VALUES, SWI_ITEMS } SWIterKind;

typedef struct {
    PyObject_HEAD
    PySwissTableObject *owner;
    size_t              pos;
    size_t              snap_size;
    SWIterKind          kind;
                                      ...
                                      PyObject *key, PyObject *value)
{
    if (!value) {
        ...
        if (rc < 0) return -1;
        if (rc == 0) { PyErr_SetObject(PyExc_KeyError, key); return -1; }
        return 0;
    }
...
static PyObject *PySwissTable_repr(PySwissTableObject *self)
{
    ...
    for (size_t i = 0; i < self->map->capacity; i++) {
        if (self->map->ctrl[i] == SW_EMPTY ||
            self->map->ctrl[i] == SW_DELETED) continue;
