...
{
    if (!it->set || !it->set->tree) return NULL;
                              ...
                              PyObject *args, PyObject *kwds)
{
    ...
    if (!self->tree) { Py_DECREF(self); return PyErr_NoMemory(); }
...
{
    if (self->tree) bst_free(self->tree);
...
{
    if (PyType_Ready(&PyBSTSetIterType) < 0) return NULL;
    ...
    it->next      = self->tree->size > 0
                    ? bst_minimum(self->tree, self->tree->root) : NULL;
    return (PyObject *)it;
}
